"""
GACE Experiment 4: MNIST Dataset - Low-Quality Client Analysis
...
def _simulate_federated_learning_with_low_quality(algorithm, low_quality_ratio):
    """Simulate federated learning with low-quality clients"""
    # Base accuracy without low-quality clients
    base_accuracies = {
        'GACE': 0.95,
        'RA': 0.85,
        'GACE-NO': 0.90,
    ...
    final_accuracy = max(0.1, min(0.95, final_accuracy))  # Clamp between 0.1 and 0.95
...
def run_complete_experiment():
    ...
    logger.info("Starting MNIST low-quality client analysis...")
...
def plot_results():
    ...
    ax.set_title('MNIST: Prediction Accuracy vs Low-Quality Client Ratio', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('exp4_mnist_results.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print results table
    print("\n" + "="*80)
    print("EXPERIMENT 4 RESULTS: MNIST Dataset - Low-Quality Client Analysis")
    print("="*80)
...
def main():
    """Main function"""
    print("="*80)
    print("GACE EXPERIMENT 4: MNIST Dataset - Low-Quality Client Analysis")
    print("="*80)
    ...
    print("Results saved to 'exp4_mnist_results.png'")
